...
class JDMatch:
  ...
  def extract_project_skills_hybrid(
    ...
    if has_bullet_embeddings and isinstance(bullet_embeddings, np.ndarray) and bullet_embeddings.ndim == 1:
        bullet_embeddings = np.expand_dims(bullet_embeddings, axis=0)
  ...
  def __init__(self,resume_skills,project_text,exp_text,achiev_text,cert_list,model,prim_skills,secod_skills,j_title,j_resp,store):
    ...
    if not resume_skills:
      ...
      self.resume_skills = [str(resume_skills)]
    ...
    if not cert_list:
      self.cert_list = []
    elif isinstance(cert_list, (list, tuple, set)):
      self.cert_list = list(cert_list)
    else:
...
"""
  ...
  def final_score(self,exp_year):
      ...
      skill_result_primary=self.compare_skill_lists(self.prim_skills,threshold=70)
      skill_result_secondry=self.compare_skill_lists(self.secod_skills,threshold=70)
      ...
      new_result["Responsibilities"]=role_resp
